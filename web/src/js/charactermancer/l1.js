    ...
    var knownSpells = function(data) {
        ...
        Object.keys(mancerdata['l1-class']['values']).filter(key => { return key.includes('utilityrow_result'); }).forEach(picked => {
            const data = mancerdata['l1-class']['values'][picked];
            const spells = data.split(',').map(entry => { return `${entry.trim()}`; });
