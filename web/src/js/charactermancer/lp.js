...
/*********************************************************************/
 ...
 //Helper function to update the text in the top bar
            ...
            _.each(asis, function(asi) {
                addRepeatingSection("choices", "asi-row", function(rowid) {
                    ...
                    getCompendiumQuery('Category:Feats', data => {
                        ...
                        counter++;
                        if(counter === asis.length) restorePreviousASI(previousState, () => { setCharmancerText({'spinner-toggle': ''}); });
                    });
                });
            });
        });
    ...
    var pickerL1 = function(eventinfo) {
        ...
        try {
            querysettings = JSON.parse(mancerdata['l1-class'].values[`${eventinfo.sourceSection}_info`]);
        } catch(e) {
            querysettings = mancerdata['l1-class'].values[`${eventinfo.sourceSection}_info`];
        };
        if(mancerdata['l1-class'].values[`${eventinfo.sourceSection}_result`]) current = mancerdata['l1-class'].values[`${eventinfo.sourceSection}_result`].split(", ");
        /*
        if(mancerdata["lp-spells"] && mancerdata["lp-spells"].repeating) {
            _.each(mancerdata["lp-spells"].repeating, function(repid) {
                if(mancerdata["lp-spells"].values[`${repid}_checked`]) {
    ...
    var closePickerL1 = function(eventinfo) {
        ...
        if(mancerdata["l1-class"].values[`${source}_info`]) {
            spellsettings = JSON.parse(mancerdata["l1-class"].values[`${source}_info`]);
        }
        ...
        if(mancerdata["l1-class"].values[`${source}_type`] !== "trait") {
            const thisclass = mancerdata["l1-class"]["data"]["class"];//_.findWhere(leveldata, {classnumber: parseInt(mancerdata["lp-choices"].values[`${source}_parent`][5])});
            newspells = {}
            _.each(mancerdata["lp-spellchoice"].repeating, function(repid) {
